...
A retail sales analysis project that loads transactional data into a MySQL 

database, validates and cleans it, and visualizes key business metrics in 
...
The raw dataset loaded into MySQL contains 493 transaction records. During 

Power BI dashboard development, 10 records with unidentified customer 

IDs were excluded, as they could not be attributed to a specific customer and 

would distort customer-level analysis (e.g., Top 10 Customers, Total Customers).
...
This difference is expected and intentional — it reflects the removal of 

orders lacking valid customer attribution, not a data loading error.
...
2\. Run `sql/database\_setup.sql` in MySQL to create the database and table

3\. Create a `.env` file with your MySQL credentials

4\. Run `notebooks/load\_to\_mysql.ipynb` to load and validate the data
